...
const pluginModule: PluginModule = {
  ...
  init(api: PluginApi) {
    ...
    function decryptAesCbcToken(encryptedHex: string, secret: string): string | null {
      try {
        const encryptedBuffer = Buffer.from(encryptedHex, 'hex');
        if (encryptedBuffer.length < 17) return null; // 至少 16 字节 IV + 1 字节数据
    ...
    (api as any).verifyAesCbcToken = verifyAesCbcToken;
